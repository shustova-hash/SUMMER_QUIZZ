...
function renderBranchInfo() {
  ...
  if (videoBox && currentBranchSettings.youtube_url) {
    const embedUrl = getYouTubeEmbedUrl(currentBranchSettings.youtube_url);
    if (embedUrl) {
      videoBox.innerHTML = `
        <iframe 
          width="100%" 
          ...
          allowfullscreen 
          style="border-radius:12px; width:100%; aspect-ratio:16/9; min-height:280px; border:none;">
        </iframe>
      `;
    }
  }
